...
3. Создать ключи API: **R2 → Manage API Tokens → Create API Token**:
   - Permissions: **Object Read & Write**, только для бакета `diversnotes-images`.
...
rclone copy b2:diversnotes-images r2:diversnotes-images --progress --transfers 16
```
...
1. В корне проекта создать `.env` по образцу `.env.example`:
   ...
   IMAGES_S3_BUCKET=diversnotes-images
...
- Аналитика: `@vercel/analytics` больше нет; включить бесплатную
  **Cloudflare Web Analytics** (Dashboard → Analytics → Web Analytics → Add site,
  вставить сниппет в `src/layouts/BaseLayout.astro`) — по желанию.
