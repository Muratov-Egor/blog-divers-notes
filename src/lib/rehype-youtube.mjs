...
function embed(id) {
  return (
    `<iframe class="yt-embed" src="https://www.youtube-nocookie.com/embed/${id}"` +
 ...
 * unknown empty element. Rewrite it into a responsive iframe embed.
  ...
  return (tree) => {
    visit(tree, "raw", (node) => {
      if (node.value?.includes("<YouTube")) {
        node.value = node.value.replace(YT_TAG, (_, id) => embed(id));
      }
    });
  };
}
